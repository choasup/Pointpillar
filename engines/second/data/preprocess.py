...
def merge_second_batch_nopadding(batch_list, _unused=False):
    #np.save("batch_list.npy", batch_list)
...
def merge_second_batch(batch_list, _unused=False):
    ...
    padding = max(example_merged['num_voxels'])[0]
    ...
    for key, elems in example_merged.items():
        if key in [
            ...
            #from IPython import embed; embed()
            voxels = []
            for i, voxel in enumerate(elems):
                need_pad = padding - voxel.shape[0]
                voxel_pad = np.pad(voxel, ((0, need_pad), (0, 0), (0, 0)), 'edge')
                voxels.append(voxel_pad)
            ret[key] = np.concatenate(voxels, axis=0)
        elif key == 'num_points':
            #from IPython import embed; embed()
            num_points = []
            for i, points in enumerate(elems):
                need_pad = padding - points.shape[0]
                points_pad = np.pad(points, ((0, need_pad)), 'edge')
                num_points.append(points_pad)
            ret[key] = np.concatenate(num_points, axis=0)
        elif key == 'match_indices_num':
            ret[key] = np.concatenate(elems, axis=0)
        ### ============= coord 0,1,2,3,4,5,6,7 ============== effect.
        elif key == 'coordinates':
            #from IPython import embed; embed()
            pad_coords = []
            for i, coord in enumerate(elems):
                need_pad = padding - coord.shape[0]
                coord_pad = np.pad(coord, ((0, need_pad), (0, 0)), 'edge')
                pad_coords.append(coord_pad)
            coors = []
            for i, coor in enumerate(pad_coords):
                coor_pad = np.pad(
                    coor, ((0, 0), (1, 0)),
                    mode='constant',
                    constant_values=i)
